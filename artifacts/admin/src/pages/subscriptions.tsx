...
import {
  useListAdminTenants,
  useGetAdminTenantSubscription,
  useUpdateAdminTenantSubscription,
  getListAdminTenantsQueryKey,
  getGetAdminTenantSubscriptionQueryKey,
...
import { Search, ExternalLink, Loader2, Save, Users, FolderOpen, HardDrive } from "lucide-react";
...
const subSchema = z.object({
  ...
  limitsOverride: z.object({
    ...
    maxStorageGb: z.coerce.number().int().min(0),
  }),
});
...
function SubscriptionEditor({ orgId, onClose }: { orgId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const overrideOrPlan = (key: "maxProjects" | "maxUsers" | "maxStorageGb") =>
      subscription.limitsOverride?.[key] ?? subscription.limits?.[key] ?? 0;

    form.reset({
      planId: subscription.planId,
      status: subscription.status as SubFormValues["status"],
      trialEndsAt: subscription.trialEndsAt ? subscription.trialEndsAt.split("T")[0] : null,
      limitsOverride: {
        maxProjects: overrideOrPlan("maxProjects") as number,
        maxUsers: overrideOrPlan("maxUsers") as number,
        maxStorageGb: overrideOrPlan("maxStorageGb") as number,
      },
    });
  }, [subscription]);
  ...
  const effectiveMaxUsers = subscription.limitsOverride?.maxUsers ?? subscription.limits?.maxUsers;
  const effectiveMaxProjects = subscription.limitsOverride?.maxProjects ?? subscription.limits?.maxProjects;
  const effectiveMaxStorage = subscription.limitsOverride?.maxStorageGb ?? subscription.limits?.maxStorageGb;
    ...
    <div className="space-y-6 mt-2">
      ...
      <div className="bg-gray-50 rounded-lg p-4 space-y-4">
        ...
        <UsageBar
          icon={FolderOpen}
          label="Projects"
          used={subscription.projectCount ?? 0}
                ...
                paginated.map((tenant) => (
                  <TableRow key={tenant.id}>
                    ...
                    <TableCell className="text-gray-500 text-sm">
                      {tenant.trialEndsAt
                        ? format(parseISO(tenant.trialEndsAt), "MMM d, yyyy")
                        : "—"}
                    </TableCell>
                    <TableCell>
                      <Button
